...
font-bold stroke  "
 ...
 absolute top-[40px]  text-4xl font-bold"
        ...
        <div
          className=" relative grid lg:grid-cols-3 grid-cols-1
     gap-[100px] mt-[50px] p-[100px]"
        >
          {data.map((item, index) => (
            <div
              key={index}
              className="relative px-[50px] bg-gray-100  py-10 rounded-lg shadow-md "
            >
              <div className="flex  md:flex-row flex-col items-center gap-[100px] justify-between">
                <div className="space-y-4">
                  <h3 className=" font-bold lg:text-3xl text-lg  text-center mt-2">
                    {item.title}
                  </h3>
                  <p className="lg:text-xl text-md  font-bold text-center mt-1">
                    {item.count}
                  </p>
                </div>
